...
interface AppointmentBookingFormProps {
    ...
    availability: DoctorAvailability[]; // Passed from parent
    onBookingSuccess: (newAppointment: any) => void; // Callback on success
    onCancel: () => void;
}
...
const AppointmentBookingForm: React.FC<AppointmentBookingFormProps> = ({
    ...
    // Calculate end_time based on start_time + fixed duration (e.g., 30 mins)
    ...
    return (
         <form onSubmit={handleSubmit} className="space-y-4">
                     ...
                     {availableSlots.length > 0 ? (
                        ...
                        >
                             ...
                             {availableSlots.map(slot => (
                                 <option key={slot} value={slot}>{formatTime(slot)}</option> // Display formatted time
                             ))}
                         </select>
                     ) : (
                        <p className="text-sm text-muted mt-1">No available slots found for this date based on general availability.</p>
                    )}
                 </div>
            )}
            ...
            <div>
                 ...
                 <div className="mt-1 flex space-x-4">
                     <label className="inline-flex items-center">
                        <input type="radio" name="appointmentType" value="in_person" checked={appointmentType === 'in_person'} onChange={(e) => setAppointmentType(e.target.value as any)} className="h-4 w-4 text-primary focus:ring-primary border-gray-300"/>
                        <span className="ml-2 text-sm text-gray-700">In-Person</span>
                     </label>
                     <label className="inline-flex items-center">
                        <input type="radio" name="appointmentType" value="virtual" checked={appointmentType === 'virtual'} onChange={(e) => setAppointmentType(e.target.value as any)} className="h-4 w-4 text-primary focus:ring-primary border-gray-300"/>
            ...
            <div className="flex justify-end space-x-3 pt-4 border-t mt-4">
                 ...
                 <button type="submit" disabled={isSubmitting || !selectedTime} className="btn-primary inline-flex justify-center px-4 py-2 text-sm font-medium disabled:opacity-50">
